...
from recipes.models import Tag, Recipe, IngredientInRecipe, Shopping_cart, Favorite, Ingredient
...
class RecipeGetSerializer(serializers.ModelSerializer):
 ...
 Для GET запросов к эндпоинтам /recipe/ и /recipe/id/.
    ...
    def get_is_in_shopping_cart(self, obj):
        ...
        return Shopping_cart.objects.filter(recipe=obj, user=request.user).exists()  
    ...
    def validate(self, data):        
        ...
        if cooking_time <= 0:
            raise serializers.ValidationError(
                {
                    'error': 'Время приготовления не должно быть менее одной минуты.'
                }
            )
        ingredients_list = []
